...
function updateUI(){
  ...
  if(currentUser){
    ...
    [a,av,b,ap,rec,timeline,followups,reports,pharmacy,diagnostics,con,ref,notifications,hop,ai,knowledge]
      .filter(Boolean)
...
function setAiMode(mode,btn){
  ...
  const placeholders={
    patient_explain:'Example: Explain my latest diagnosis and medicines in simple language.',
    patient_questions:'Example: What should I ask my doctor about my recent consultation?',
    patient_summary:'You can leave this blank to summarize your MediBridge record.',
    doctor_reference:'Example: Summarize the key guideline considerations for this clinical question.',
    doctor_patient_review:'Example: Review the consented record for important issues, missing information, or medication/allergy concerns.'
  };
...
async function loadAiPage(){
  ...
  if(role==='patient'){
    ...
    document.getElementById('aiPageSubtitle').textContent='Understand your MediBridge record and prepare better questions for your clinician.';
    document.getElementById('aiSafetyText').textContent='This assistant can explain and summarize your own record. It does not replace a clinician or emergency care.';
    ...
    document.getElementById('aiPageSubtitle').textContent='Clinical reference and second-look support using only data you are already allowed to access.';
    document.getElementById('aiSafetyText').textContent='AI suggestions require clinician verification. Patient-specific review is restricted to active consent scopes.';
...
async function askMediBridgeAI(){
  ...
  btn.disabled=true;
  btn.textContent='Thinking...';
  msg('aiMessage','Preparing authorized context and contacting the AI service...');
  ...
  try{
    const {data,error}=await supabaseClient.functions.invoke('medibridge-ai',{
      body:{
        ...
        use_clinical_knowledge: currentProfile?.role==='doctor' ? Boolean(document.getElementById('useClinicalKnowledge')?.checked) : false
      }
    });
    ...
    document.getElementById('aiAnswer').innerHTML=renderAiMarkdown(data.answer||'No response generated.');
    document.getElementById('aiContextBadge').textContent=
      data.sources?.length?'Grounded reference':(data.used_patient_context?'Authorized record context':'General reference');
    document.getElementById('aiAnswerCard').classList.remove('hidden');
    ...
    msg('aiMessage','AI response generated. Verify important clinical information before use.','success');
  }catch(err){
    msg('aiMessage',err?.message||'AI request failed.','error');
  }finally{
    btn.disabled=false;
...
function clearAiAnswer(){
  ...
  document.getElementById('aiSources').classList.add('hidden');
