...
let sessionEpoch=0;
let authResolutionState='resolving';
let sessionRestoreSlowTimer=null;
let initialSessionRestorePromise=null;
let sessionRestoreAttempt=0;
let lastSessionResumeRetryAt=0;
const PROFILE_CACHE_TTL_MS=30000;
const SESSION_RESTORE_SLOW_MS=12000;
const SESSION_RESUME_RETRY_THROTTLE_MS=2000;
const externalScriptPromises=new Map();
const DISCOVERY_CACHE_TTL_MS=60000;
const discoveryCache=new Map();
let careDiscoveryRequestId=0;
let emergencyDiscoveryRequestId=0;
let diagnosticDiscoveryRequestId=0;
let patientSubjects=[];
...
const PAGE_META={
  ...
  ai:['MediBridge AI | MediBridge','MediBridge AI supports understanding and preparation; it does not replace clinical care.'],
...
function patientFriendlyError(error,fallback='This information could not be loaded. Please try again.'){
  const raw=String(error?.message||error||'').toLowerCase();
  if(/network|fetch|connection|timeout/.test(raw))return 'Check your connection and try again.';
  if(/jwt|session|sign.?in|auth/.test(raw))return 'Your session needs to be refreshed. Sign in again and retry.';
  if(/permission|policy|row.level|not.allowed|forbidden|unauthor/.test(raw))return 'This information is not available to the current account.';
  return fallback;
}

function trustedProviderText(value,fallback){
  const text=String(value||'').trim();
  if(!text)return fallback;
  if(obviousPlaceholderValue(text))return fallback;
  return text;
}

function clearSessionRestoreSlowTimer(){
  if(sessionRestoreSlowTimer){
    clearTimeout(sessionRestoreSlowTimer);
    sessionRestoreSlowTimer=null;
  }
}

function sessionPerformanceStart(){
  return typeof performance!=='undefined' && typeof performance.now==='function'
    ? performance.now()
    : Date.now();
}

function recordSessionPerformance(stage,startedAt){
  const duration=Math.max(0,sessionPerformanceStart()-startedAt);
  const store=window.__MEDIBRIDGE_SESSION_PERFORMANCE__||{};
  store[stage]=Math.round(duration);
  store.recorded_at=new Date().toISOString();
  window.__MEDIBRIDGE_SESSION_PERFORMANCE__=store;
  return duration;
}

function setSessionConnectionNotice(message=''){
  const notice=document.getElementById('sessionConnectionNotice');
  const text=document.getElementById('sessionConnectionMessage');
  if(text && message)text.textContent=message;
  notice?.classList.toggle('hidden',!message);
}

function dashboardVisualHint(user=window.__MEDIBRIDGE_SESSION_HINT__){
  if(!user)return null;
  const metadata=user.user_metadata||{};
  const allowedRoles=new Set(['patient','doctor','mental_health','hospital','pharmacy','lab','admin']);
  const proposedRole=String(user.role||metadata.role||'').toLowerCase();
  const role=allowedRoles.has(proposedRole)?proposedRole:'patient';
  return {role,full_name:String(user.full_name||metadata.full_name||metadata.name||'').trim(),email:String(user.email||'').trim()};
}

function showDashboardVisualShell(user=window.__MEDIBRIDGE_SESSION_HINT__){
  const hint=dashboardVisualHint(user);
  if(!hint)return;
  document.getElementById('dashGate')?.classList.add('hidden');
  document.getElementById('dashContent')?.classList.remove('hidden');
  document.getElementById('verifyBadge')?.classList.add('hidden');
  document.getElementById('patientFollowupDashboardCard')?.classList.add('hidden');
  document.getElementById('doctorConsentRecordsCard')?.classList.add('hidden');
  document.getElementById('dashboardNextStepCard')?.classList.add('hidden');
  const title=document.getElementById('dashTitle');
  if(title)title.textContent=hint.full_name?`Welcome back, ${hint.full_name.split(/\s+/)[0]}`:'Your MediBridge';
  const cards=document.getElementById('dashCards');
  if(cards){
    cards.innerHTML=dashboardActionsForRole(hint.role).map(([route,label,detail,icon],index)=>`
      <button class="dashboard-action-card session-shell-card ${index===0?'primary-dashboard-action':''}" type="button" disabled aria-disabled="true">
        <span class="dashboard-action-icon" aria-hidden="true">${NAV_ICONS[route]||icon}</span>
        <span class="dashboard-action-copy"><b>${escapeAdmin(label)}</b><small>${escapeAdmin(detail)}</small></span>
      </button>
    `).join('');
  }
}

function showDashboardSignedOutState(){
  const gate=document.getElementById('dashGate');
  const loading=document.getElementById('dashGateLoading');
  const signedOut=document.getElementById('dashGateSignedOut');
...
function beginSessionResolution(){
  authResolutionState='resolving';
  clearSessionRestoreSlowTimer();
  document.body?.classList.remove('auth-resolving','auth-profile-loading');
  document.getElementById('mainContent')?.setAttribute('aria-busy','false');
  document.getElementById('sessionRestoreOverlay')?.setAttribute('aria-hidden','true');
  document.getElementById('loginBtn')?.classList.add('hidden');
  document.getElementById('logoutBtn')?.classList.add('hidden');
  document.getElementById('sessionLabel')?.classList.add('hidden');
  setSessionConnectionNotice('');
  if(window.__MEDIBRIDGE_SESSION_HINT__){
    activatePageView('dashboard',{skipHistory:true,focus:false});
    showDashboardVisualShell();
  }
  sessionRestoreSlowTimer=setTimeout(()=>{
    if(authResolutionState!=='resolving')return;
    setSessionConnectionNotice('Your account is taking longer to connect. You can retry without signing out.');
  },SESSION_RESTORE_SLOW_MS);
}

function showAuthenticatedShell(user){
  if(!user)return;
  authResolutionState='authenticated-loading';
  clearSessionRestoreSlowTimer();
  document.body?.classList.remove('guest-mode');
  document.body?.classList.remove('auth-resolving');
  document.body?.classList.add('auth-profile-loading');
  document.getElementById('mainContent')?.setAttribute('aria-busy','false');
  document.getElementById('sessionRestoreOverlay')?.setAttribute('aria-hidden','true');
  document.getElementById('loginBtn')?.classList.add('hidden');
  ...
  if(sessionLabel){
    sessionLabel.textContent=user.email||'Your account';
    sessionLabel.classList.remove('hidden');
  }
  document.getElementById('adaptiveDesktopNav')?.classList.add('guest-nav-hidden');
  document.getElementById('mobileBottomNav')?.classList.add('guest-nav-hidden');
  document.getElementById('mobileMenuBtn')?.classList.add('guest-nav-hidden');
  document.getElementById('headerNotificationsBtn')?.classList.add('hidden');
  document.getElementById('headerAiBtn')?.classList.add('hidden');
  document.getElementById('patientSubjectBar')?.classList.add('hidden');

  if(currentPageName==='home'){
    activatePageView('dashboard',{skipHistory:true,focus:false});
  }
  showDashboardVisualShell(user);
  setSessionConnectionNotice('');
  sessionRestoreSlowTimer=setTimeout(()=>{
    if(authResolutionState!=='authenticated-loading')return;
    setSessionConnectionNotice('Your account information is taking longer to connect. You can retry without signing out.');
  },SESSION_RESTORE_SLOW_MS);
}

function settleSessionResolution(signedIn){
  authResolutionState=signedIn?'authenticated':'signed-out';
  clearSessionRestoreSlowTimer();
  document.body?.classList.remove('auth-resolving','auth-profile-loading');
  document.getElementById('mainContent')?.setAttribute('aria-busy','false');
  document.getElementById('sessionRestoreOverlay')?.setAttribute('aria-hidden','true');
  document.documentElement.classList.remove('mb-session-hint','mb-no-session-hint');
  window.__MEDIBRIDGE_SESSION_HINT__=null;
  setSessionConnectionNotice('');
  if(!signedIn && currentPageName==='dashboard')showDashboardSignedOutState();
}

function showSessionResolutionError(error){
  authResolutionState='error';
  clearSessionRestoreSlowTimer();
  document.body?.classList.remove('auth-resolving','auth-profile-loading');
  document.getElementById('mainContent')?.setAttribute('aria-busy','false');
  document.getElementById('sessionRestoreOverlay')?.setAttribute('aria-hidden','true');
  setSessionConnectionNotice(patientFriendlyError(error,'MediBridge could not connect to your account. Check your connection and retry.'));
}

function invalidateInFlightSessionInitialization(){
  sessionEpoch+=1;
  sessionInitPromise=null;
  lastHandledSessionFingerprint=null;
  profileLoadPromise=null;
  profileLoadUserId=null;
}

async function restoreInitialSession(force=false,{automatic=false}={}){
  // Never start a second token/session request while the first one is active.
  // Overlapping refresh attempts are slower on mobile Safari and can let an
  // older response overwrite a newer authenticated state.
  if(initialSessionRestorePromise)return initialSessionRestorePromise;
  if(force){
    invalidateInFlightSessionInitialization();
  }
  if(currentUser)showAuthenticatedShell(currentUser);
  else beginSessionResolution();
  const attempt=++sessionRestoreAttempt;
  const restoration=(async()=>{
    const sessionStartedAt=sessionPerformanceStart();
    const {data,error}=await supabaseClient.auth.getSession();
    recordSessionPerformance('get_session_ms',sessionStartedAt);
    if(error)throw error;
    await handleSession(data.session,{source:automatic?'automatic-retry':'initial-restore'});
    return data.session;
  })();
  initialSessionRestorePromise=restoration;
  try{
    return await restoration;
  }catch(error){
    if(attempt!==sessionRestoreAttempt)return null;
    console.error('Session restoration failed.',error);
    window.medibridgeOperationalEvent?.('session_restoration_failed','auth');
    showSessionResolutionError(error);
    return null;
  }finally{
    if(initialSessionRestorePromise===restoration)initialSessionRestorePromise=null;
  }
}

async function retrySessionRestore(options={}){
  await restoreInitialSession(true,options);
}

function resumeInterruptedSessionRestoration(){
  if(document.visibilityState==='hidden')return;
  // A visibility/pageshow event must not branch into a second bootstrap while
  // the original request is still resolving. Retry only after a real error.
  if(authResolutionState!=='error')return;
  const now=Date.now();
  if(now-lastSessionResumeRetryAt<SESSION_RESUME_RETRY_THROTTLE_MS)return;
  lastSessionResumeRetryAt=now;
  retrySessionRestore({automatic:true}).catch(()=>{});
}
...
function activatePageView(n,options={}){
...
function showPage(n,options={}){
  ...
  if(!activatePageView(n,options))return;
...
function setAuthMode(m){
  ...
  document.getElementById('authSubmit').textContent=m==='login'?'Sign in':'Create account';
...
const lineIcon=body=>`<svg class="mb-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${body}</svg>`;
const NAV_ICONS={
  home:lineIcon('<path d="m3 11 9-7 9 7"/><path d="M5 10v10h14V10"/><path d="M9 20v-6h6v6"/>'),
  dashboard:lineIcon('<path d="m3 11 9-7 9 7"/><path d="M5 10v10h14V10"/><path d="M9 20v-6h6v6"/>'),
  book:lineIcon('<circle cx="12" cy="12" r="9"/><path d="M12 8v8M8 12h8"/>'),
  appointments:lineIcon('<rect x="3" y="5" width="18" height="16" rx="3"/><path d="M8 3v4M16 3v4M3 10h18M8 14h3M8 17h6"/>'),
  records:lineIcon('<path d="M6 3h9l4 4v14H6z"/><path d="M14 3v5h5M9 12h6M9 16h6"/>'),
  ai:lineIcon('<path d="m12 3 1.3 4.2L17.5 8.5l-4.2 1.3L12 14l-1.3-4.2-4.2-1.3 4.2-1.3z"/><path d="m18.5 14 .7 2.3 2.3.7-2.3.7-.7 2.3-.7-2.3-2.3-.7 2.3-.7z"/>'),
  family:lineIcon('<circle cx="9" cy="8" r="3"/><path d="M3.5 20v-2a5.5 5.5 0 0 1 11 0v2M16 5.5a3 3 0 0 1 0 5.8M17 14a5 5 0 0 1 3.5 4.8V20"/>'),
  timeline:lineIcon('<circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/>'),
  careplans:lineIcon('<rect x="5" y="4" width="14" height="17" rx="2"/><path d="M9 4V2h6v2M8 12l2 2 5-5M9 18h6"/>'),
  followups:lineIcon('<path d="M20 11a8 8 0 1 0-2.3 5.7"/><path d="M20 5v6h-6"/>'),
  reports:lineIcon('<path d="M6 3h9l4 4v14H6z"/><path d="M14 3v5h5M9 13h6M9 17h4"/>'),
  pharmacy:lineIcon('<path d="m8.5 15.5 7-7a3.5 3.5 0 1 1 5 5l-7 7a3.5 3.5 0 0 1-5-5z"/><path d="m12 12 5 5"/>'),
  diagnostics:lineIcon('<path d="M9 3h6M10 3v6l-5 9a2 2 0 0 0 1.8 3h10.4A2 2 0 0 0 19 18l-5-9V3M8 15h8"/>'),
  consent:lineIcon('<path d="M12 3 5 6v5c0 4.6 2.8 8.2 7 10 4.2-1.8 7-5.4 7-10V6z"/><path d="m9 12 2 2 4-4"/>'),
  referrals:lineIcon('<path d="M7 7h11M15 4l3 3-3 3M17 17H6M9 14l-3 3 3 3"/>'),
  profile:lineIcon('<circle cx="12" cy="8" r="4"/><path d="M4 21a8 8 0 0 1 16 0"/>'),
  availability:lineIcon('<circle cx="12" cy="12" r="9"/><path d="M12 7v5l4 2"/>'),
  hospitalops:lineIcon('<path d="M4 21V5h16v16M8 9h8M8 13h8M10 21v-4h4v4"/>'),
  admin:lineIcon('<path d="M12 3 5 6v5c0 4.6 2.8 8.2 7 10 4.2-1.8 7-5.4 7-10V6z"/><path d="M12 8v4M12 16h.01"/>'),
  knowledge:lineIcon('<path d="M4 5.5A3.5 3.5 0 0 1 7.5 2H20v17H7.5A3.5 3.5 0 0 0 4 22z"/><path d="M4 5.5V22"/>'),
  auth:lineIcon('<path d="M10 17l5-5-5-5M15 12H3M14 4h6v16h-6"/>'),
  emergency:lineIcon('<path d="M12 3v18M3 12h18"/>'),
  mental:lineIcon('<path d="M20.8 5.7a5.5 5.5 0 0 0-7.8 0L12 6.8l-1-1.1a5.5 5.5 0 0 0-7.8 7.8L12 22l8.8-8.5a5.5 5.5 0 0 0 0-7.8z"/>'),
  notifications:lineIcon('<path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9M10 21h4"/>'),
  privacy:lineIcon('<path d="M12 3 5 6v5c0 4.6 2.8 8.2 7 10 4.2-1.8 7-5.4 7-10V6z"/><path d="M9 12h6"/>')
};
...
function syncAdaptiveNavigation(){
  ...
  const adaptiveDesktopNav=document.getElementById('adaptiveDesktopNav');
  const mobileMenuButton=document.getElementById('mobileMenuBtn');
  const sessionLabel=document.getElementById('sessionLabel');

  document.body?.classList.toggle('guest-mode',!currentUser);
  adaptiveDesktopNav?.classList.toggle('guest-nav-hidden',!currentUser);
  mobileMenuButton?.classList.toggle('guest-nav-hidden',!currentUser);
  sessionLabel?.classList.toggle('hidden',!currentUser);
  ...
  let mobilePrimary=[];
  if(mobile){
    mobile.classList.toggle('guest-nav-hidden',!currentUser);
    if(!currentUser){
...
// encodeURIComponent leaves apostrophes unescaped. Encode them before a
// value is placed inside a single-quoted legacy inline event argument.
...
function showPasswordResetPage(){
  if(!activatePageView('password-reset',{skipHistory:true}))return;
...
async function logout(){
  ...
  currentProfile=null;
  profileLoadedAt=0;
  lastHandledSessionFingerprint='signed-out';
...
function resetSensitiveClientState(){
  cancelMediBridgeAiRequest();
  ...
  selectedConsentedPatient=null;
  selectedPatientConsultationAppointmentId=null;
  selectedPatientConsultationPayload=null;
  doctorOverviewPatientId=null;
  doctorOverviewSubjectId=null;
  doctorOverviewAppointmentId=null;
  currentDoctorAccessRequest=null;
  cachedDoctors=[];
  cachedHospitals=[];
  cachedVerifiedLabs=[];
  cachedVerifiedPharmacies=[];
  patientPharmacyPageData={consultations:[],items:[],requests:[]};
  mentalProvidersCache=[];
  mentalProviderSessionCache=new Map();
  emergencyHospitalResultCache.clear();
  discoveryCache.clear();
  careDiscoveryRequestId+=1;
  emergencyDiscoveryRequestId+=1;
  diagnosticDiscoveryRequestId+=1;
  lastReliableDeviceLocation=null;
  lastReliableDeviceLocationAt=0;
  ...
  carePlaceDebounce=null;
  diagnosticPlaceDebounce=null;
  patientPharmacyPlaceDebounce=null;
  const sensitiveContainers=[
    'aiChatThread','careTimelineList','medicalRecordList','patientPharmacyRequests',
    'patientDiagnosticRequests','mentalPatientAppointments','mentalConsentList',
    'sharedPatientRecordList','appointmentDetailCard','appointmentFiles',
    'appointmentLinkedReports','notificationList'
  ];
  sensitiveContainers.forEach(id=>document.getElementById(id)?.replaceChildren());
  document.getElementById('notificationCountBadge')?.classList.add('hidden');
  document.getElementById('headerNotificationBadge')?.classList.add('hidden');
}

function isMissingBootstrapRpc(error){
  const code=String(error?.code||'');
  const message=String(error?.message||'').toLowerCase();
  return ['PGRST202','42883'].includes(code) ||
    (/get_my_session_bootstrap/.test(message) && /not found|schema cache|does not exist/.test(message));
}

async function loadAuthoritativeSessionBootstrap(expectedUserId){
  if(!expectedUserId || currentUser?.id!==expectedUserId)return null;
  const startedAt=sessionPerformanceStart();
  const {data,error}=await supabaseClient.rpc('get_my_session_bootstrap');

  if(currentUser?.id!==expectedUserId)return null;
  if(!error && data?.profile?.id===expectedUserId){
    recordSessionPerformance('bootstrap_rpc_ms',startedAt);
    return {
      profile:data.profile,
      patientSubjects:Array.isArray(data.patient_subjects)?data.patient_subjects:[],
      source:'combined-rpc'
    };
  }

  // Backward-compatible adapter for a staged frontend deployment. Production
  // should install the v40.5 RPC first so login remains one database request.
  if(error && !isMissingBootstrapRpc(error))throw error;

  const fallbackStartedAt=sessionPerformanceStart();
  const [profileResult,subjectsResult]=await Promise.all([
    supabaseClient
      ...
      .single(),
    supabaseClient.rpc('list_my_patient_subjects')
  ]);
  recordSessionPerformance('bootstrap_fallback_ms',fallbackStartedAt);
  if(profileResult.error)throw profileResult.error;
  if(subjectsResult.error)throw subjectsResult.error;
  return {
    profile:profileResult.data,
    patientSubjects:Array.isArray(subjectsResult.data)?subjectsResult.data:[],
    source:'parallel-fallback'
  };
}

async function handleSession(s,{forceProfile=false,source='auth-event'}={}){
  const incomingUser=s?.user||null;
  // Token refreshes change expires_at but not the account. Fingerprinting by
  // expiry caused a complete profile/family/dashboard reload every refresh.
  const fingerprint=incomingUser?.id||'signed-out';

  if(fingerprint===lastHandledSessionFingerprint && sessionInitPromise){
  ...
  if(
    ...
    currentUser?.id===(incomingUser?.id||null) &&
    (incomingUser ? Boolean(currentProfile) : authResolutionState==='signed-out')
  ){
    return;
  }
  ...
  const accountChanged=(currentUser?.id||null)!==(incomingUser?.id||null);
  if(accountChanged){
    resetSensitiveClientState();
    currentProfile=null;
    profileLoadedAt=0;
  ...
  if(currentUser && (accountChanged || !currentProfile || authResolutionState!=='authenticated')){
    showAuthenticatedShell(currentUser);
  }

  const initializationStartedAt=sessionPerformanceStart();
  const initialization=(async()=>{
    const expectedUserId=incomingUser?.id||null;

    if(currentUser){
      const bootstrap=(accountChanged || forceProfile || !currentProfile)
        ? await loadAuthoritativeSessionBootstrap(expectedUserId)
        : {profile:currentProfile,patientSubjects,source:'memory'};

      if(epoch!==sessionEpoch || currentUser?.id!==expectedUserId || !bootstrap)return;
      currentProfile=bootstrap.profile;
      profileLoadedAt=Date.now();

      if(currentProfile?.role==='patient'){
        await loadPatientSubjects(Promise.resolve({data:bootstrap.patientSubjects,error:null}));
      }else{
        patientSubjects=[];
        activePatientSubjectId=null;
      }
    ...
    if(epoch!==sessionEpoch || (currentUser?.id||null)!==expectedUserId)return;
    updateUI();

    if(currentPageName==='dashboard'){
      if(currentUser)await refreshDashboard({identityReady:true});
      else showDashboardSignedOutState();
    }

    settleSessionResolution(Boolean(currentUser));
    recordSessionPerformance('total_bootstrap_ms',initializationStartedAt);
    window.__MEDIBRIDGE_SESSION_PERFORMANCE__.source=source;

    // The Privacy page can be opened while the initial auth session is still
    // resolving. Re-render it after the authoritative state is available.
    if(currentPageName==='privacy')await loadPrivacyCenter();
    ...
    if(currentUser){
      // Non-critical watchers begin only after the first usable screen.
  ...
  sessionInitPromise=initialization;
  ...
  try{
    ...
    if(epoch===sessionEpoch && (currentUser?.id||null)===(incomingUser?.id||null)){
      showSessionResolutionError(error);
    }
    throw error;
  }finally{
    if(sessionInitPromise===initialization)sessionInitPromise=null;
    if(!initializationSucceeded && lastHandledSessionFingerprint===fingerprint){
      lastHandledSessionFingerprint=null;
    }
  }
}

async function loadProfile(force=false,expectedUserId=currentUser?.id){
  if(!expectedUserId || currentUser?.id!==expectedUserId)return null;
  ...
  if(profileLoadPromise && profileLoadUserId===expectedUserId)return profileLoadPromise;

  profileLoadUserId=expectedUserId;
  const request=(async()=>{
...
async function loadPatientSubjects(prefetchedRequest=null){
  ...
  const firstResult=prefetchedRequest
    ? await prefetchedRequest
    : await supabaseClient.rpc('list_my_patient_subjects');
  let {data,error}=firstResult||{};

  // Existing accounts already have a Self subject. Avoid a redundant write RPC
  // during every login; initialise only when the authoritative list proves it is
  // missing, then re-read once.
  if(!error && !(data||[]).some(subject=>subject.is_self)){
    ...
    if(ensureError){
      ...
      ({data,error}=await supabaseClient.rpc('list_my_patient_subjects'));
    }
  }
...
function renderPatientSubjectSwitcher(){
  ...
  if(rel){
    ...
    rel.classList.toggle('hidden',Boolean(subject?.is_self));
  }
}
...
function toggleFamilyCreateForm(open){
  const form=document.getElementById('familyCreateForm');
  if(!form)return;
  form.classList.toggle('hidden',!open);
  if(open){
    form.scrollIntoView({behavior:window.matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth',block:'start'});
    setTimeout(()=>document.getElementById('familyMemberName')?.focus(),0);
  }
}
...
async function refreshNotificationBadge(){
  ...
  try{
    ...
    if(error || currentUser?.id!==expectedUserId)return;
...
async function refreshDashboard({identityReady=false}={}){let g=document.getElementById('dashGate'),c=document.getElementById('dashContent');if(!currentUser){g.classList.remove('hidden');c.classList.add('hidden');return}if(!identityReady){await loadProfile();if(currentProfile?.role==='patient'&&!patientSubjects.length)await loadPatientSubjects();}g.classList.add('hidden');c.classList.remove('hidden');document.getElementById('dashboardNextStepCard')?.classList.remove('hidden');let r=currentProfile?.role||'patient';document.getElementById('dashTitle').textContent=r==='patient'?`Managing ${activePatientSubjectName()}`:'Welcome, '+(currentProfile?.full_name||currentUser.email);const verifyBadge=document.getElementById('verifyBadge');verifyBadge.textContent=currentProfile?.verification_status||'pending';verifyBadge.classList.toggle('hidden',r==='patient');const cards=dashboardActionsForRole(r);
...
async function loadPatientProfile(){
  ...
  const authorityWarning=document.getElementById('profileAuthorityWarning');
  const birthDate=data.date_of_birth?new Date(`${data.date_of_birth}T00:00:00`):null;
  const age=birthDate && !Number.isNaN(birthDate.getTime())
    ? Math.floor((Date.now()-birthDate.getTime())/31557600000)
    : null;
  const selfMinor=Boolean(data.is_self && age!==null && age<18);
  if(authorityWarning){
    authorityWarning.classList.toggle('hidden',!selfMinor);
    authorityWarning.innerHTML=selfMinor
      ? '<b>Account authority needs review</b><p>This Self profile has a date of birth under 18. A minor must be managed by a parent or legal guardian; do not use real health data until the account authority and profile relationship are corrected.</p>'
      : '';
  }
}
...
function loadExternalScriptOnce(key,url){
  ...
  const promise=new Promise((resolve,reject)=>{
    let existing=document.querySelector(`script[data-medibridge-lazy="${key}"]`);
    ...
    return `<div class="card doctor-card">
      <div class="row between">
        <div>
          ...
          <h2>${escapeAdmin(trustedProviderText(d.full_name,'Verified doctor'))}</h2>
        ...
        <div class="row">
          ${mapLink?`<button class="btn secondary" onclick="openEncodedExternalUrl('${encodeInlineArgument(mapLink)}','google_maps')">Location</button>`:''}
          ${directionsLink?`<button class="btn secondary" onclick="openEncodedExternalUrl('${encodeInlineArgument(directionsLink)}','google_maps')">Directions</button>`:''}
    ...
    return `<div class="card hospital-result">
      <div class="row between">
        <div>
          ...
          <h2>${escapeAdmin(trustedProviderText(h.hospital_name,'Verified hospital'))}</h2>
...
function openHospitalBooking(id){
  ...
  activatePageView('book-hospital');
...
async function loadMyAppointments(){
  ...
  if(role==='patient'){
    ...
    const section=(title,rows,emptyCopy)=>`<section class="appointment-group appointment-group-${title.toLowerCase()}" aria-label="${title}">
      <h2 class="appointment-group-title">${title}</h2>
      <div class="appointment-group-list">${rows.length?rows.map(renderAppointmentCard).join(''):`<div class="card compact-empty-state"><p class="muted">${emptyCopy}</p></div>`}</div>
    </section>`;
...
async function openAppointmentDetail(id){
  ...
  activatePageView('appointment-detail');
...
async function openDoctorPatientOverview(patientId,appointmentId){
  ...
  activatePageView('doctor-patient-overview');
...
async function showIncomingVideoCall(session){
  ...
  requestAnimationFrame(()=>modal.querySelector('.accept-call')?.focus());
}
  ...
  box.innerHTML=data.map(f=>{
    ...
    return `<div class="card" style="margin-top:10px">
      <div class="row between">
        ...
        <div class="row">
          <button class="btn secondary" onclick="openEncodedAppointmentFile('${encodeInlineArgument(f.file_path)}')">Open</button>
          ${mine?`<button class="btn secondary" onclick="deleteEncodedAppointmentFile('${f.id}','${encodeInlineArgument(f.file_path)}')">Delete</button>`:''}
        </div>
      </div>
    </div>`;
  }).join('');
...
function openEncodedAppointmentFile(encodedPath){
  try{return openAppointmentFile(decodeURIComponent(encodedPath))}catch(_){return false}
}

function deleteEncodedAppointmentFile(id,encodedPath){
  try{return deleteAppointmentFile(id,decodeURIComponent(encodedPath))}catch(_){return false}
}
    ...
    return `
      <div class="appointment-report-row">
        ...
        <div class="appointment-report-actions">
          <button class="btn secondary" onclick="openMedicalReport('${r.id}','${encodeInlineArgument(r.file_path)}')">Open report</button>
        ...
        <div class="followup-actions">
          ...
          ${r.status==='pending'
            ? `<button class="btn" onclick="rebookFromFollowup('${r.id}')">Book follow-up</button>
               <button class="btn secondary" onclick="dismissFollowup('${r.id}')">Hide reminder</button>`
            : ''}
        </div>
      </div>`;
  }).join('');
...
async function dismissFollowup(reminderId){
  if(!confirm('Hide this reminder? This does not cancel care or notify the doctor.'))return;
      ...
      <div class="report-card">
        <div class="row between">
          <div>
            ...
            <p class="muted">${escapeAdmin(when)}${!r.title&&r.file_name?` · ${escapeAdmin(r.file_name)}`:''}</p>
          </div>
        </div>

        ${r.notes?`<p class="report-note"><b>Notes:</b> ${escapeAdmin(r.notes)}</p>`:''}
        ...
        <div class="report-actions">
          ...
          ${currentProfile.role==='patient'
            ...
            : ''}
          ${r.uploaded_by===currentUser.id?`<button class="btn secondary" onclick="deleteMedicalReport('${r.id}','${encodeInlineArgument(r.file_path)}')">Delete</button>`:''}
        </div>
      </div>
    `;
  }).join('');
...
function renderHealthRecordEditor(tab){
  ...
  const templates={
    allergies:`<div class="health-entry-form">
      <input id="hrAllergen" aria-label="Allergen" placeholder="Allergen">
      <input id="hrReaction" aria-label="Allergy reaction" placeholder="Reaction">
      <select id="hrSeverity" aria-label="Allergy severity"><option value="">Severity</option><option value="mild">Mild</option><option value="moderate">Moderate</option><option value="severe">Severe</option></select>
      <button class="btn" onclick="addHealthRecordItem('allergies')">Add allergy</button>
    </div>`,
    conditions:`<div class="health-entry-form">
      <input id="hrCondition" aria-label="Condition" placeholder="Condition">
      <input id="hrConditionDate" aria-label="Condition diagnosis date" type="date">
      <input id="hrConditionNotes" aria-label="Condition notes" placeholder="Notes">
      <button class="btn" onclick="addHealthRecordItem('conditions')">Add condition</button>
    </div>`,
    medications:`<div class="health-entry-form">
      <input id="hrMedicine" aria-label="Medicine" placeholder="Medicine">
      <input id="hrMedicineStrength" aria-label="Medicine strength" placeholder="Strength">
      <input id="hrMedicineDose" aria-label="Medicine dose" placeholder="Dose">
      <input id="hrMedicineFrequency" aria-label="Medicine frequency" placeholder="Frequency">
      <button class="btn" onclick="addHealthRecordItem('medications')">Add medication</button>
    </div>`,
    surgeries:`<div class="health-entry-form">
      <input id="hrSurgery" aria-label="Procedure or surgery" placeholder="Procedure / surgery">
      <input id="hrSurgeryDate" aria-label="Surgery date" type="date">
      <input id="hrSurgeryHospital" aria-label="Surgery hospital" placeholder="Hospital">
      <button class="btn" onclick="addHealthRecordItem('surgeries')">Add surgery</button>
    </div>`,
    immunizations:`<div class="health-entry-form">
      <input id="hrVaccine" aria-label="Vaccine" placeholder="Vaccine">
      <input id="hrDoseLabel" aria-label="Vaccine dose" placeholder="Dose">
      <input id="hrVaccineDate" aria-label="Vaccination date" type="date">
      <button class="btn" onclick="addHealthRecordItem('immunizations')">Add immunization</button>
    </div>`,
    family:`<div class="health-entry-form">
      <input id="hrRelation" aria-label="Family relation" placeholder="Relation">
      <input id="hrFamilyCondition" aria-label="Family health condition" placeholder="Condition">
      <input id="hrFamilyNotes" aria-label="Family history notes" placeholder="Notes">
    ...
    vitals:`<div class="health-entry-form vitals-form">
      <input id="hrSys" aria-label="Systolic blood pressure" type="number" placeholder="Systolic">
      <input id="hrDia" aria-label="Diastolic blood pressure" type="number" placeholder="Diastolic">
      <input id="hrPulse" aria-label="Pulse" type="number" placeholder="Pulse">
      <input id="hrSpo2" aria-label="Blood oxygen saturation" type="number" placeholder="SpO₂">
      <input id="hrTemp" aria-label="Temperature in Celsius" type="number" step="0.1" placeholder="Temp °C">
      <input id="hrWeight" aria-label="Weight in kilograms" type="number" step="0.1" placeholder="Weight kg">
      <input id="hrHeight" aria-label="Height in centimetres" type="number" step="0.1" placeholder="Height cm">
...
async function loadCarePlans(){
  ...
  if(!data?.length){
    list.innerHTML=`<div class="card empty-state">
      ...
      ${currentProfile.role==='patient'
        ? '<button class="btn secondary" type="button" onclick="showPage(\'appointments\')">View appointments</button>'
        : ''}
    </div>`;
    return;
  }
...
async function openReferralDetail(id){
  ...
  activatePageView('referral-detail');
...
async function openPatientConsultationExplain(appointmentId){
  ...
  activatePageView('consultation-explain');
...
let mentalProviderSessionCache=new Map();
...
async function loadMentalProviderAppointments(){if(!currentUser||currentProfile?.role!=='mental_health')return;const {data,error}=await supabaseClient.rpc('get_my_mental_health_sessions');if(error){mentalProviderSessionCache=new Map();mentalProviderAppointments.innerHTML='<p class="error">Session requests could not be loaded. Please try again.</p>';return}mentalProviderSessionCache=new Map((data||[]).map(a=>[a.id,a]));if(!data?.length){mentalProviderAppointments.innerHTML='<div class="empty-state"><p>No session requests yet.</p></div>';return}mentalProviderAppointments.innerHTML=data.map(a=>`<div class="mental-session-row"><div><b>${escapeAdmin(a.subject_name||'Patient')}</b><p>${escapeAdmin(formatDateTime(a.requested_start))} · ${a.consultation_type==='online'?'Online':'In person'}</p><p class="muted">${escapeAdmin(mentalConcernLabel(a.concern_category))}</p>${a.patient_note?`<p>${escapeAdmin(a.patient_note)}</p>`:''}</div><div class="mental-session-actions">${healthcareStatusBadge(a.status)}${a.status==='requested'?`<button class="btn" onclick="setMentalAppointmentStatus('${a.id}','confirmed')">Accept</button><button class="btn secondary" onclick="setMentalAppointmentStatus('${a.id}','declined')">Decline</button>`:''}${a.status==='confirmed'?`<button class="btn" onclick="openMentalRecordEditor('${a.id}')">Session record</button><button class="btn secondary" onclick="viewSharedMentalHistory('${a.subject_id}','${a.id}')">Shared history</button><button class="btn secondary" onclick="setMentalAppointmentStatus('${a.id}','completed')">Complete</button>`:''}${a.status==='completed'?`<button class="btn secondary" onclick="openMentalRecordEditor('${a.id}')">View / edit record</button><button class="btn secondary" onclick="viewSharedMentalHistory('${a.subject_id}','${a.id}')">Shared history</button>`:''}</div></div>`).join('')}
async function viewSharedMentalHistory(subjectId,appointmentId){const {data,error}=await supabaseClient.rpc('get_shared_mental_health_summaries',{target_subject:subjectId});if(error){toast(error.message,'error');return}const name=mentalProviderSessionCache.get(appointmentId)?.subject_name||'Patient',rows=data||[];if(!rows.length){toast(`No shared mental-health summaries are available for ${name}.`);return}const box=document.getElementById('mentalProviderRecordEditor');mentalProviderRecordTitle.textContent=`Shared history — ${name}`;mentalRecordAppointmentId.value='';mentalPatientSummary.readOnly=true;mentalCarePlan.readOnly=true;mentalFollowupDate.disabled=true;mentalPrivateNote.readOnly=true;document.getElementById('mentalPrivateNoteBox').classList.add('hidden');document.getElementById('mentalRecordSaveBtn').classList.add('hidden');mentalPatientSummary.value=rows.map((r,i)=>`${i+1}. ${r.provider_name||'Professional'} — ${r.patient_summary||'No summary'}${r.follow_up_date?`\nFollow-up: ${r.follow_up_date}`:''}`).join('\n\n');mentalCarePlan.value=rows.map(r=>r.care_plan).filter(Boolean).join('\n\n');mentalFollowupDate.value='';mentalPrivateNote.value='';box.classList.remove('hidden');msg('mentalRecordMessage','Read-only shared history loaded through explicit mental-health consent and logged access.','success')}
...
let mediBridgeAiRequestId=0;


function cancelMediBridgeAiRequest(){
  mediBridgeAiRequestId+=1;
  mediBridgeAiAbortController?.abort();
  mediBridgeAiAbortController=null;
  mediBridgeAiRequestInFlight=false;
  mediBridgeAiGenerationStopped=false;
  const send=document.getElementById('aiAskBtn');
...
function aiConversationKey(){
  const context=currentProfile?.role==='patient'
    ? patientSubjectId()||'self'
    : currentAiMode==='doctor_patient_review'
      ? document.getElementById('doctorAiPatientSelect')?.value||'no-patient'
      : 'provider';
  return `${currentUser?.id||'guest'}:${currentProfile?.role||'guest'}:${context}:${currentAiMode}`;
}
...
function resizeAiComposer(){
  const input=document.getElementById('aiPrompt');
  if(!input)return;
  input.style.height='auto';
  input.style.height=Math.min(Math.max(input.scrollHeight,48),128)+'px';
}
...
function setAiMode(mode,btn){
  if(mode!==currentAiMode)cancelMediBridgeAiRequest();
  currentAiMode=mode;
...
async function loadAiPage(){
  ...
  if(role==='patient'){
    ...
    const patientSelect=document.getElementById('doctorAiPatientSelect');
    if(patientSelect && !patientSelect.dataset.aiChangeBound){
      patientSelect.dataset.aiChangeBound='1';
  ...
  if(prompt && !prompt.dataset.aiEnterBound){
    ...
    prompt.addEventListener('input',resizeAiComposer);
...
async function askMediBridgeAI(){
  ...
  const requestId=++mediBridgeAiRequestId;
  const conversationKey=aiConversationKey();
  const isCurrentRequest=()=>requestId===mediBridgeAiRequestId && conversationKey===aiConversationKey();
  ...
  try{
    ...
    if(err?.code==='MEDIBRIDGE_AI_CANCELLED' || mediBridgeAiGenerationStopped){
      msg('aiMessage','Generation stopped.','');
      return;
    }
    ...
    if(requestId===mediBridgeAiRequestId){
      mediBridgeAiAbortController=null;
      mediBridgeAiRequestInFlight=false;
      mediBridgeAiGenerationStopped=false;
      btn.disabled=false;
...
async function loadConsentDoctorOptions(){
  const select=document.getElementById('consentDoctorSelect');
  const grantButton=document.getElementById('grantDoctorConsentBtn');
  const {data:profiles,error:pErr}=await supabaseClient
    .from('profiles')
    .select('id,full_name')
    .eq('role','doctor')
  ...
  if(pErr){
    if(select)select.innerHTML='<option value="">Doctors could not be loaded</option>';
    if(grantButton)grantButton.disabled=true;
    msg('consentMessage',patientFriendlyError(pErr,'Verified doctors could not be loaded. Please try again.'),'error');
    return;
  }

  if(!profiles?.length){
    if(select)select.innerHTML='<option value="">No verified doctors available</option>';
    if(grantButton)grantButton.disabled=true;
    msg('consentMessage','No verified doctor is currently available for a new access grant.','');
    return;
  }
  ...
  if(dErr){
    if(select)select.innerHTML='<option value="">Doctor details could not be loaded</option>';
    if(grantButton)grantButton.disabled=true;
    msg('consentMessage',patientFriendlyError(dErr,'Doctor details could not be loaded. Please try again.'),'error');
    return;
  }
  ...
  select.innerHTML=
    profiles.map(p=>{
      const d=dm[p.id]||{};
      return `<option value="${p.id}">${escapeAdmin(trustedProviderText(p.full_name,'Verified doctor'))} — ${escapeAdmin(d.specialty||'General')}${d.hospital_name?' · '+escapeAdmin(trustedProviderText(d.hospital_name,'Verified facility')):''}</option>`;
    }).join('');
}
...
async function grantDoctorConsent(){
  ...
  if(error){
    msg('consentMessage',patientFriendlyError(error,'Access could not be granted. Please review the selection and try again.'),'error');
    return;
  }
...
async function loadActiveConsents(){
  const {data,error}=await supabaseClient
    .from('record_consents')
    .select('id,doctor_id,scopes,status,granted_at,expires_at,revoked_at')
    .eq('patient_id',currentUser.id)
    .eq('subject_id',patientSubjectId())
    .order('granted_at',{ascending:false});
  ...
  if(error){
    box.innerHTML=`<div class="compact-empty-state"><p class="error">${escapeAdmin(patientFriendlyError(error,'Access permissions could not be loaded. Please try again.'))}</p></div>`;
    return;
  }

  if(!data?.length){
    box.innerHTML='<div class="compact-empty-state"><p class="muted">No doctor currently has record access for this family profile.</p></div>';
    return;
  }
  ...
  const rows=data.map(c=>{
    ...
    const scopeLabels=(c.scopes||[]).map(s=>CONSENT_SCOPE_LABELS[s]||s);
    ...
    return {...c,effective,markup:`<div class="mini-row">
      <div>
        <b>${escapeAdmin(names[c.doctor_id]||'Doctor')}</b>
        <div class="consent-scope-chips">${scopeLabels.length
          ? scopeLabels.map(label=>`<span>${escapeAdmin(label)}</span>`).join('')
          : '<span>No record types selected</span>'}</div>
        <small>Expires: ${escapeAdmin(expires)}</small>
      </div>
      <div class="row">
        ${healthcareStatusBadge(effective)}
        ${effective==='active'
  ...
  const active=rows.filter(c=>c.effective==='active');
  const previous=rows.filter(c=>c.effective!=='active');
  box.innerHTML=`<div class="consent-access-group"><h3>Active (${active.length})</h3>${active.length?active.map(c=>c.markup).join(''):'<p class="muted">No active doctor access.</p>'}</div>
    ${previous.length?`<details class="consent-history-disclosure"><summary>Previous or expired access (${previous.length})</summary>${previous.map(c=>c.markup).join('')}</details>`:''}`;
}
...
async function revokeConsent(id){
  ...
  if(error){
    msg('consentMessage',patientFriendlyError(error,'Access could not be revoked. Please try again.'),'error');
    return;
  }
...
async function loadRecordAccessHistory(){
  const {data,error}=await supabaseClient
    ...
    .select('id,accessor_id,access_type,source,accessed_at')
    .eq('patient_id',currentUser.id)
    .eq('subject_id',patientSubjectId())
  ...
  if(error){
    box.innerHTML=`<div class="compact-empty-state"><p class="error">${escapeAdmin(patientFriendlyError(error,'Record-view history could not be loaded. Please try again.'))}</p></div>`;
    return;
  }

  if(!data?.length){
    box.innerHTML='<div class="compact-empty-state"><p class="muted">No record views have been logged for this family profile.</p></div>';
    return;
  }
...
async function openConsentedPatientRecord(patientId,consentId){
  ...
  activatePageView('shared-patient-records');
...
async function loadConsentedRecordSections(patientId,consent){
  ...
  if(scopes.has('appointment_files')){
    ...
    box.innerHTML+=`<div class="card"><h2>Reports / files</h2>${
      files.length
        ? files.map(f=>`<div class="mini-row"><div><b>${escapeAdmin(f.file_name)}</b></div><button class="btn secondary" onclick="openEncodedConsentedFile('${encodeInlineArgument(f.file_path)}','${patientId}','${consent.id}','${subjectId}')">Open</button></div>`).join('')
...
function openEncodedConsentedFile(encodedPath,patientId,consentId,subjectId){
  try{return openConsentedFile(decodeURIComponent(encodedPath),patientId,consentId,subjectId)}catch(_){return false}
}
...
function openEmergencyArrivalNotice(id){
  ...
  activatePageView('emergency-notice');
...
function renderEmergencyHospitalResults(results,context){
  ...
  box.innerHTML=results.map(h=>{
    ...
    return `<div class="card hospital-result">
      ...
      <div class="row">
        ${mapsLink?`<button class="btn secondary" onclick="openEncodedExternalUrl('${encodeInlineArgument(mapsLink)}','google_maps')">Location</button>`:''}
        ${directionsLink?`<button class="btn" onclick="openEncodedExternalUrl('${encodeInlineArgument(directionsLink)}','google_maps')">Directions</button>`:''}
...
async function loadPatientDiagnosticsPage(){
  ...
  if(labErr){
    document.getElementById('diagnosticCentresList').innerHTML=`<div class="empty-state"><h3>Diagnostic centres could not be loaded</h3><p class="error">${escapeAdmin(patientFriendlyError(labErr))}</p><button class="btn secondary" type="button" onclick="loadPatientDiagnosticsPage()">Try again</button></div>`;
    return;
  }
  ...
  if(reqErr){
    box.innerHTML=`<div class="compact-empty-state"><p class="error">${escapeAdmin(patientFriendlyError(reqErr,'Diagnostic requests could not be loaded. Please try again.'))}</p></div>`;
    return;
  }
...
async function renderDiagnosticDiscovery(){
  ...
  if(!box)return;
  const requestId=++diagnosticDiscoveryRequestId;
  ...
  if(!rows.length){
    box.innerHTML='<div class="empty-state"><div class="empty-icon" aria-hidden="true">🧪</div><h3>No participating diagnostic centre found</h3><p class="muted">Try another city or district, or increase the GPS radius.</p></div>';
    return;
  }
  ...
  if(requestId!==diagnosticDiscoveryRequestId || currentPageName!=='diagnostics')return;

  if(error){
    box.innerHTML=`<div class="empty-state"><h3>Test catalogues could not be loaded</h3><p class="error">${escapeAdmin(patientFriendlyError(error))}</p></div>`;
    return;
  }
        ...
        <div class="row between">
          <div>
            <div class="diagnostic-title-row">
              ...
              <div>
                <h3>${escapeAdmin(trustedProviderText(l.lab_name,'Verified diagnostic centre'))}</h3>
          ...
          <div class="row">
            ...
            ${centreMapLink?`<button class="btn secondary" onclick="openEncodedExternalUrl('${encodeInlineArgument(centreMapLink)}','google_maps')">Open Maps</button>`:''}
          </div>
        </div>
...
async function sendDiagnosticRequest(labId){
  ...
  if(error){
    toast('Request could not be sent',patientFriendlyError(error,'Review the selected tests and try again.'));
    return;
  }
...
async function renderPatientDiagnosticRequests(requests,box){
  if(!requests.length){
    box.innerHTML='<div class="compact-empty-state"><p class="muted">No diagnostic requests yet. Select tests from a participating centre above to start one.</p></div>';
    return;
  }
  ...
  const [{data:labs},{data:items},{data:reports}]=await Promise.all([
    ...
    supabaseClient.from('diagnostic_request_items').select('id,request_id,test_id,test_name,category,indicative_price').in('request_id',reqIds),
  ...
  box.innerHTML=requests.map(r=>{
    ...
    return `
      <div class="diagnostic-request-card">
        ...
        ${r.requested_date?`<p><b>Requested for:</b> ${escapeAdmin(timelineDisplayDate(r.requested_date,true))}${r.preferred_time?` · ${escapeAdmin(formatDiagnosticTime(r.preferred_time))}`:''}</p>`:''}
        ${r.lab_note?`<p><b>Centre note:</b> ${escapeAdmin(r.lab_note)}</p>`:''}

        <div class="row diagnostic-patient-actions">
          ${report?`<button class="btn" onclick="openMedicalReport('${report.id}','${encodeInlineArgument(report.file_path)}')">Open report</button>`:''}
...
async function cancelDiagnosticRequest(requestId){
  ...
  if(error){
    toast('Request could not be cancelled',patientFriendlyError(error,'Please try again.'));
    return;
  }
      ...
      <div class="diagnostic-request-card">
        ...
        <div class="pharmacy-inbox-actions">
          ...
          ${r.status==='in_process' && !report
            ...
            : ''}
          ${report
            ? `<button class="btn secondary" onclick="openMedicalReport('${report.id}','${encodeInlineArgument(report.file_path)}')">Open published report</button>`
            : ''}
        ...
        ${r.status==='in_process' && !report?`
          <div id="diagnosticReportUpload-${r.id}" class="diagnostic-report-upload hidden">
            ...
            <input id="diagnosticReportFile-${r.id}" aria-label="Patient diagnostic report file" type="file" accept="application/pdf,image/jpeg,image/png,image/webp">
            <input id="diagnosticReportNote-${r.id}" aria-label="Optional report note for patient" placeholder="Optional note for patient">
            <div class="row">
...
function renderPatientPharmacyDiscovery(){
  ...
  if(!rows.length){
    box.innerHTML='<div class="empty-state"><div class="empty-icon" aria-hidden="true">＋</div><h3>No participating pharmacy found</h3><p class="muted">Try another city or district, or increase the GPS radius.</p></div>';
    return;
  }
  ...
  box.innerHTML=rows.map((p,index)=>{
    ...
    <div class="pharmacy-discovery-card">
      ...
      <div class="pharmacy-discovery-main">
        <h3>${escapeAdmin(trustedProviderText(p.pharmacy_name,'Verified pharmacy'))}</h3>
        ...
        <div class="row" style="margin-top:8px">
          ${pharmacyMapLink?`<button class="btn secondary" onclick="openEncodedExternalUrl('${encodeInlineArgument(pharmacyMapLink)}','google_maps')">Open Maps</button>`:''}
        </div>
      </div>
    </div>
  `;
...
async function loadPatientPharmacyPage(){
  ...
  if(cErr){
    list.innerHTML=`<div class="compact-empty-state"><p class="error">${escapeAdmin(patientFriendlyError(cErr,'Prescriptions could not be loaded. Please try again.'))}</p></div>`;
    return;
  }

  if(pharmacyResult.error){
    list.innerHTML=`<div class="compact-empty-state"><p class="error">${escapeAdmin(patientFriendlyError(pharmacyResult.error,'Participating pharmacies could not be loaded. Please try again.'))}</p></div>`;
    return;
  }
  ...
  if(rErr){
    requestsBox.innerHTML=`<div class="compact-empty-state"><p class="error">${escapeAdmin(patientFriendlyError(rErr,'Pharmacy requests could not be loaded. Please try again.'))}</p></div>`;
    return;
  }
...
function renderPatientPrescriptionFulfilment(){
  ...
  if(!withRx.length){
    list.innerHTML='<div class="empty-state"><div class="empty-icon" aria-hidden="true">▤</div><h3>No prescription is ready to send</h3><p class="muted">A prescription recorded during a consultation will appear here automatically.</p><button class="btn secondary" type="button" onclick="showPage(\'appointments\')">View appointments</button></div>';
    return;
  }
  ...
  const hasUnsentPrescription=withRx.some(c=>!requestByConsultation[c.id]);
  const pharmacyLocationBanner=!hasUnsentPrescription?'':!locationChosen
    ? '<div class="prescription-location-banner"><b>Choose one location to send a prescription</b><p class="muted">Enter a city or district, or use your current location in the Find a pharmacy section above.</p></div>'
    : !pharmacies.length
      ? '<div class="prescription-location-banner"><b>No participating pharmacy in this location</b><p class="muted">Try another city or district, or increase the GPS radius.</p></div>'
      : '';

  list.innerHTML=pharmacyLocationBanner+withRx.map(c=>{
    ...
    }else if(!locationChosen||!pharmacies.length){
      fulfilmentArea='';
    }else{
      fulfilmentArea=`
...
async function renderPatientPharmacyRequests(requests,box){
  if(!requests.length){
    box.innerHTML='<div class="compact-empty-state"><p class="muted">No pharmacy requests yet. Send a doctor-recorded prescription to start one.</p></div>';
    return;
  }
...
async function sendPrescriptionToPharmacy(consultationId){
  ...
  if(error){
    toast('Prescription could not be sent',patientFriendlyError(error,'Please review the request and try again.'));
    return;
  }
...
supabaseClient.auth.onAuthStateChange((event,session)=>{
  // Supabase can emit TOKEN_REFRESHED and repeated SIGNED_IN events while an
  // iPad tab resumes. The access token updates inside the client automatically;
  // rebuilding profile, family and dashboard state here only adds latency.
  const sameReadyAccount=Boolean(
    session?.user?.id &&
    session.user.id===currentUser?.id &&
    currentProfile &&
    authResolutionState==='authenticated'
  );
  if(event==='TOKEN_REFRESHED' && sameReadyAccount)return;
  if(event==='SIGNED_IN' && sameReadyAccount)return;
  ...
  handleSession(session,{
    ...
    window.medibridgeOperationalEvent?.('session_initialization_failed','auth');
  });
...
window.addEventListener('pageshow',event=>{
  if(event.persisted)resumeInterruptedSessionRestoration();
});
document.addEventListener('visibilitychange',resumeInterruptedSessionRestoration);
