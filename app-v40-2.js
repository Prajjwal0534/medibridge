...
function setSessionRestoreMessage(title,detail,{retry=false}={}){
  const titleNode=document.getElementById('sessionRestoreTitle');
  const detailNode=document.getElementById('sessionRestoreDetail');
  const retryButton=document.getElementById('sessionRestoreRetryBtn');
  if(titleNode)titleNode.textContent=title;
  if(detailNode)detailNode.textContent=detail;
  retryButton?.classList.toggle('hidden',!retry);
}

function showDashboardRestoreState(){
...
function beginSessionResolution(){
  ...
  sessionRestoreAutoRetryCount=0;
  clearSessionRestoreSlowTimer();
  document.body?.classList.add('auth-resolving');
  document.body?.classList.remove('auth-profile-loading');
  document.getElementById('mainContent')?.setAttribute('aria-busy','true');
  document.getElementById('sessionRestoreOverlay')?.setAttribute('aria-hidden','false');
  document.getElementById('loginBtn')?.classList.add('hidden');
  document.getElementById('logoutBtn')?.classList.add('hidden');
  document.getElementById('sessionLabel')?.classList.add('hidden');
  setSessionRestoreMessage('Restoring your secure session…','Checking this device for an existing MediBridge sign-in.');
  sessionRestoreSlowTimer=setTimeout(()=>{
    setSessionRestoreMessage(
      'Still restoring your session…',
      'This is taking longer than expected. Check your connection or retry safely.',
      {retry:true}
    );
...
function showAuthenticatedShell(user){
  ...
  clearSessionRestoreSlowTimer();
  document.body?.classList.remove('auth-resolving','guest-mode');
  ...
  if(sessionLabel){
    sessionLabel.textContent=(user.email||'Your account')+' · Restoring account';
  ...
  if(currentPageName==='dashboard')showDashboardRestoreState();

  const dashboardMessage=document.getElementById('dashRestoreMessage');
  if(dashboardMessage)dashboardMessage.textContent='Loading your profile and family healthcare…';
  const dashboardRetry=document.getElementById('dashRestoreRetryBtn');
  dashboardRetry?.classList.add('hidden');
  sessionRestoreSlowTimer=setTimeout(()=>{
    if(authResolutionState!=='authenticated-loading')return;
    if(sessionRestoreAutoRetryCount<SESSION_RESTORE_AUTO_RETRIES){
      sessionRestoreAutoRetryCount+=1;
      if(dashboardMessage)dashboardMessage.textContent='Your session is secure. Reconnecting account information automatically…';
      retrySessionRestore({automatic:true}).catch(()=>{});
      return;
    }
    if(dashboardMessage)dashboardMessage.textContent='Your session is secure. Account information is taking longer than expected.';
    dashboardRetry?.classList.remove('hidden');
  },SESSION_RESTORE_SLOW_MS);
}
...
function showSessionResolutionError(error){
  ...
  setSessionRestoreMessage(
    ...
    patientFriendlyError(error,'Check your connection and retry. Your account has not been signed out.'),
    {retry:true}
  );
}
...
async function restoreInitialSession(force=false,{automatic=false}={}){
  if(initialSessionRestorePromise && !force)return initialSessionRestorePromise;
  if(force){
    invalidateInFlightSessionInitialization();
    if(!automatic)sessionRestoreAutoRetryCount=0;
  }
...
function resumeInterruptedSessionRestoration(){
  ...
  if(!['resolving','error','authenticated-loading'].includes(authResolutionState))return;
  const now=Date.now();
...
async function handleSession(s){
  ...
  if(
    ...
    if(authResolutionState==='resolving'){
      if(currentUser){
        showAuthenticatedShell(currentUser);
        updateUI();
        if(currentPageName==='dashboard')await refreshDashboard();
        settleSessionResolution(true);
      }else{
        updateUI();
        settleSessionResolution(false);
      }
    }
    return;
  }
  ...
  const shouldShowRestoreShell=Boolean(
    currentUser && (
      accountChanged ||
      !currentProfile ||
      ['resolving','error','signed-out'].includes(authResolutionState)
    )
  );
  if(shouldShowRestoreShell)showAuthenticatedShell(currentUser);

  const initialization=(async()=>{
    const expectedUserId=incomingUser?.id||null;

    if(currentUser){
      await loadProfile(true,expectedUserId);
    }else{
      currentProfile=null;
      profileLoadedAt=0;
    }
    ...
    if(currentPageName==='dashboard'){
      if(currentUser)await refreshDashboard();
    ...
    // resolving. Re-render it after the session becomes authoritative so a
    // signed-in person never sees stale "Sign in" controls.
    if(currentPageName==='privacy'){
...
async function loadPatientSubjects(){
  ...
  let {data,error}=await supabaseClient.rpc('list_my_patient_subjects');
