...
function syncAdaptiveNavigation(){
  ...
  if(desktopMore){
    desktopMore.innerHTML=renderGroupedSecondaryMenu(model.secondary);
  }

  moreBtn?.classList.toggle('hidden',model.secondary.length===0);

  if(mobile){
    if(!currentUser){
      mobile.innerHTML=model.primary.map(([r,l])=>navButtonMarkup(r,l,{mobile:true})).join('');
    }else{
      const mobilePrimary=model.primary.filter(([route])=>route!=='profile').slice(0,4);
      mobile.innerHTML=
  ...
  if(mobileMenu){
    const allSecondary=[
      ...model.secondary,
      ...(currentUser && !model.primary.some(([r])=>r==='profile')?[['profile','Profile']]:[])
    ];
...
async function saveMentalHealthProviderProfile(e){e.preventDefault();const mapsRaw=val('mentalMapsLink'),maps=mapsRaw?safeGoogleMapsUrl(mapsRaw):null;if(mapsRaw&&!maps){msg('profileMessage','Use a valid HTTPS Google Maps link.','error');return}let path=null,file=mentalVerificationFile.files[0];if(file){const ext=(file.name.split('.').pop()||'').toLowerCase();if(!['application/pdf','image/jpeg','image/png'].includes(file.type)||!['pdf','jpg','jpeg','png'].includes(ext)){msg('mentalVerificationStatus','Use PDF, JPG or PNG only.','error');return}if(file.size>10*1024*1024){msg('mentalVerificationStatus','Maximum file size is 10 MB.','error');return}path=`${currentUser.id}/${Date.now()}_${file.name.replace(/[^a-zA-Z0-9._-]/g,'_')}`;msg('mentalVerificationStatus','Uploading...');const {error}=await supabaseClient.storage.from('Mental health verification').upload(path,file);if(error){msg('mentalVerificationStatus',error.message,'error');return}}
...
async function saveDoctor(e){e.preventDefault();
  ...
  let path=null,file=document.getElementById('doctorCertificate').files[0];if(file){let allowed=['application/pdf','image/jpeg','image/png'];let certExt=(file.name.split('.').pop()||'').toLowerCase();let allowedCertExt=['pdf','jpg','jpeg','png'];if(!allowed.includes(file.type)||!allowedCertExt.includes(certExt)){msg('certificateStatus','Use PDF, JPG or PNG only.','error');return}if(file.size>10*1024*1024){msg('certificateStatus','Maximum 10 MB.','error');return}path=currentUser.id+'/'+Date.now()+'_'+file.name.replace(/[^a-zA-Z0-9._-]/g,'_');msg('certificateStatus','Uploading...');let {error}=await supabaseClient.storage.from('Doctor verification').upload(path,file);if(error){msg('certificateStatus',error.message,'error');return}msg('certificateStatus','Certificate uploaded privately.','success')}let dLat=val('doctorLatitude'),dLng=val('doctorLongitude');
...
async function ensureJitsiProviderLoaded(){
  ...
  await loadExternalScriptOnce('jitsi','https://meet.jit.si/external_api.js');
...
async function loadMyAppointments(){
  ...
  if(!data?.length){
    list.innerHTML=`<div class="card"><b>No appointments for ${escapeAdmin(role==='patient'?activePatientSubjectName():'this account')}.</b></div>`;
...
async function loadHospitalAppointments(){
  ...
  if(!data?.length){
    box.innerHTML=`<div class="card"><b>No hospital appointments${role==='patient'?` for ${escapeAdmin(activePatientSubjectName())}`:''}.</b></div>`;
    return;
  }
  ...
  box.innerHTML=data.map(x=>{
    ...
    return `<div class="card">
      <div class="row between"><div><div class="row appointment-person-row"><h2 style="margin:0">${escapeAdmin(title)}</h2><span class="subject-inline">${escapeAdmin(context)}</span></div><p><b>${when}</b></p><p class="muted">${escapeAdmin(x.department||'General appointment')} · ${escapeAdmin(x.reason_for_visit||'No reason provided')}</p></div><span class="badge">${escapeAdmin(x.status)}</span></div>
...
async function openEmbeddedVideoRoom(session){
  ...
  try{
    msg('videoSessionMessage','Preparing secure video room...');
    await ensureJitsiProviderLoaded();
  }catch(error){
    msg('videoSessionMessage','Video provider could not load. Check internet connection and try again.','error');
    return;
  }
...
let mentalProvidersCache=[],mentalSelectedConcern='talk_to_someone';
...
async function loadMentalHealthAppointments(){if(!currentUser||currentProfile?.role!=='patient')return;const {data,error}=await supabaseClient.from('mental_health_appointments').select('id,provider_id,requested_start,consultation_type,concern_category,status').eq('patient_id',currentUser.id).eq('subject_id',patientSubjectId()).order('requested_start',{ascending:false});if(error){mentalPatientAppointments.innerHTML=`<p class="error">${escapeAdmin(error.message)}</p>`;return}if(!data?.length){mentalPatientAppointments.innerHTML='<div class="empty-state"><p>No mental-health sessions for this family profile yet.</p></div>';return}const ids=[...new Set(data.map(x=>x.provider_id))],{data:profiles}=await supabaseClient.from('profiles').select('id,full_name').in('id',ids),names=Object.fromEntries((profiles||[]).map(x=>[x.id,x.full_name])),{data:summaries}=await supabaseClient.from('mental_health_session_summaries').select('appointment_id,patient_summary,care_plan,follow_up_date').in('appointment_id',data.map(x=>x.id)),sm=Object.fromEntries((summaries||[]).map(x=>[x.appointment_id,x]));mentalPatientAppointments.innerHTML=data.map(a=>{const s=sm[a.id];return `<div class="mental-session-row"><div><b>${escapeAdmin(names[a.provider_id]||'Mental-health professional')}</b><p>${escapeAdmin(formatDateTime(a.requested_start))} · ${a.consultation_type==='online'?'Online':'In person'}</p><p class="muted">${escapeAdmin(mentalConcernLabel(a.concern_category))}</p>${s?.patient_summary?`<div class="mental-session-summary"><b>Session summary</b><p>${escapeAdmin(s.patient_summary)}</p>${s.care_plan?`<p><b>Next steps:</b> ${escapeAdmin(s.care_plan)}</p>`:''}${s.follow_up_date?`<p><b>Follow-up:</b> ${escapeAdmin(s.follow_up_date)}</p>`:''}</div>`:''}</div><div class="mental-session-actions"><span class="badge">${escapeAdmin(a.status)}</span>${['requested','confirmed'].includes(a.status)?`<button class="btn secondary" onclick="cancelMentalAppointment('${a.id}')">Cancel</button>`:''}</div></div>`}).join('')}
...
async function loadMentalProviderAppointments(){if(!currentUser||currentProfile?.role!=='mental_health')return;const {data,error}=await supabaseClient.rpc('get_my_mental_health_sessions');if(error){mentalProviderAppointments.innerHTML=`<p class="error">${escapeAdmin(error.message)}</p>`;return}if(!data?.length){mentalProviderAppointments.innerHTML='<div class="empty-state"><p>No session requests yet.</p></div>';return}mentalProviderAppointments.innerHTML=data.map(a=>`<div class="mental-session-row"><div><b>${escapeAdmin(a.subject_name||'Patient')}</b><p>${escapeAdmin(formatDateTime(a.requested_start))} · ${a.consultation_type==='online'?'Online':'In person'}</p><p class="muted">${escapeAdmin(mentalConcernLabel(a.concern_category))}</p>${a.patient_note?`<p>${escapeAdmin(a.patient_note)}</p>`:''}</div><div class="mental-session-actions"><span class="badge">${escapeAdmin(a.status)}</span>${a.status==='requested'?`<button class="btn" onclick="setMentalAppointmentStatus('${a.id}','confirmed')">Accept</button><button class="btn secondary" onclick="setMentalAppointmentStatus('${a.id}','declined')">Decline</button>`:''}${a.status==='confirmed'?`<button class="btn" onclick="openMentalRecordEditor('${a.id}','${encodeURIComponent(a.subject_name||'Patient')}')">Session record</button><button class="btn secondary" onclick="viewSharedMentalHistory('${a.subject_id}','${encodeURIComponent(a.subject_name||'Patient')}')">Shared history</button><button class="btn secondary" onclick="setMentalAppointmentStatus('${a.id}','completed')">Complete</button>`:''}${a.status==='completed'?`<button class="btn secondary" onclick="openMentalRecordEditor('${a.id}','${encodeURIComponent(a.subject_name||'Patient')}')">View / edit record</button><button class="btn secondary" onclick="viewSharedMentalHistory('${a.subject_id}','${encodeURIComponent(a.subject_name||'Patient')}')">Shared history</button>`:''}</div></div>`).join('')}
...
async function openMentalRecordEditor(id,name){mentalRecordAppointmentId.value=id;mentalProviderRecordTitle.textContent=decodeURIComponent(name||'Session');mentalPatientSummary.readOnly=false;mentalCarePlan.readOnly=false;mentalFollowupDate.disabled=false;mentalPrivateNote.readOnly=false;document.getElementById('mentalPrivateNoteBox').classList.remove('hidden');document.getElementById('mentalRecordSaveBtn').classList.remove('hidden');mentalProviderRecordEditor.classList.remove('hidden');const [{data:s},{data:p}]=await Promise.all([supabaseClient.from('mental_health_session_summaries').select('patient_summary,care_plan,follow_up_date').eq('appointment_id',id).maybeSingle(),supabaseClient.from('mental_health_private_notes').select('private_note').eq('appointment_id',id).maybeSingle()]);mentalPatientSummary.value=s?.patient_summary||'';mentalCarePlan.value=s?.care_plan||'';mentalFollowupDate.value=s?.follow_up_date||'';mentalPrivateNote.value=p?.private_note||'';mentalProviderRecordEditor.scrollIntoView({behavior:'smooth',block:'start'})}
function closeMentalRecordEditor(){mentalProviderRecordEditor.classList.add('hidden')}
async function saveMentalSessionRecord(){const id=val('mentalRecordAppointmentId');const {data:a,error:aErr}=await supabaseClient.from('mental_health_appointments').select('patient_id,subject_id,provider_id').eq('id',id).eq('provider_id',currentUser.id).single();if(aErr){msg('mentalRecordMessage',aErr.message,'error');return}const {error:sErr}=await supabaseClient.from('mental_health_session_summaries').upsert({appointment_id:id,patient_id:a.patient_id,subject_id:a.subject_id,provider_id:currentUser.id,patient_summary:val('mentalPatientSummary')||null,care_plan:val('mentalCarePlan')||null,follow_up_date:val('mentalFollowupDate')||null},{onConflict:'appointment_id'});if(sErr){msg('mentalRecordMessage',sErr.message,'error');return}const privateText=val('mentalPrivateNote');if(privateText){const {error:pErr}=await supabaseClient.from('mental_health_private_notes').upsert({appointment_id:id,provider_id:currentUser.id,private_note:privateText},{onConflict:'appointment_id'});if(pErr){msg('mentalRecordMessage',pErr.message,'error');return}}msg('mentalRecordMessage','Session record saved. Private notes remain separate.','success')}
...
async function searchEmergencyHospitals({force=false}={}){
  ...
  msg('emergencyMessage','Searching nearby hospitals...');
  msg('emergencyResultsMessage','');
  ...
  if(radiusValue!=='district' && !emergencyUserLocation){
    ...
    box.innerHTML=`<div class="card empty-state">
      ...
      <p class="muted">Tap “Use my location” above for nearby hospitals with distance and directions.</p>
    </div>`;
  ...
  if(error){
    document.getElementById('emergencyHospitalResults').innerHTML=
      `<div class="card"><b>Could not load emergency hospitals.</b><p class="muted">${escapeAdmin(error.message)}</p></div>`;
    msg('emergencyResultsMessage','Please try Refresh.','error');
    return;
  }
  ...
  box.innerHTML=results.map(h=>{
    ...
    return `<div class="card hospital-result">
      <div class="row between">
        ...
        <span class="badge status-${escapeAdmin(status)}">${isFallback?'status unconfirmed':escapeAdmin(status)}</span>
      </div>
...
async function setMentalHealthVerification(id,status){if(!['verified','rejected'].includes(status))return;if(!confirm(`${status==='verified'?'Approve':'Reject'} this mental-health professional?`))return;const {error}=await supabaseClient.from('profiles').update({verification_status:status}).eq('id',id).eq('role','mental_health');if(error){alert(error.message);return}await loadPendingMentalHealthProviders()}
