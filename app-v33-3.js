...
async function switchPatientSubject(subjectId){
  ...
  const patientPages=new Set([
    ...
    'reports','pharmacy','diagnostics','consent','referrals','emergency','ai','family'
  ]);
...
async function loadHospitalAppointments(){
  ...
  let q=supabaseClient.from('hospital_appointments').select('*').order('requested_start',{ascending:true});
...
let emergencyView='search';

function setEmergencyView(view){
  emergencyView=view==='hospitals'?'hospitals':'search';
  const isSearch=emergencyView==='search';
  document.getElementById('emergencySearchTab')?.classList.toggle('active',isSearch);
  document.getElementById('emergencyHospitalsTab')?.classList.toggle('active',!isSearch);
  document.getElementById('emergencySearchTab')?.setAttribute('aria-selected',String(isSearch));
  document.getElementById('emergencyHospitalsTab')?.setAttribute('aria-selected',String(!isSearch));
  document.getElementById('emergencySearchPanel')?.classList.toggle('hidden',!isSearch);
  document.getElementById('emergencyHospitalsPanel')?.classList.toggle('hidden',isSearch);
}

function updateEmergencyResultCount(count){
  const badge=document.getElementById('emergencyResultCount');
  if(!badge)return;
...
async function loadEmergencyPage(){
  ...
  setEmergencyView('search');
  updateEmergencyResultCount(emergencyHospitalResultCache?.size||0);
...
async function runEmergencySearch(){
  ...
  setEmergencyView('hospitals');
}

async function refreshEmergencyResults(){
  msg('emergencyResultsMessage','Refreshing emergency hospitals...');
  await searchEmergencyHospitals();
  setEmergencyView('hospitals');
}
...
async function useEmergencyLocation(){
  ...
  try{
    ...
    msg(
      'emergencyMessage',
      'Location added. Matching hospitals will show distance and be sorted using current GPS location.',
      'success'
    );
...
async function searchEmergencyHospitals(){
  ...
  if(radiusValue!=='district' && emergencyUserLocation){
    ...
    if(error){
      ...
      updateEmergencyResultCount(0);
      msg('emergencyResultsMessage','Please try Refresh, or return to Search and adjust the filters.','error');
      return;
    }
  ...
  if(radiusValue!=='district' && !emergencyUserLocation){
    ...
    updateEmergencyResultCount(0);
    msg('emergencyResultsMessage','Use your location from the Search tab or choose District-wide.','error');
    return;
  }
  ...
  if(!district){
    ...
    updateEmergencyResultCount(0);
    msg('emergencyResultsMessage','Enter a district on the Search tab for district-wide search.','error');
    return;
  }
  ...
  renderEmergencyHospitalResults(data||[],{mode:'district',district});
}
...
function renderEmergencyHospitalResults(results,context){
  ...
  updateEmergencyResultCount(results?.length||0);

  if(!results.length){
    box.innerHTML='<div class="card"><b>No hospital with a recently verified matching emergency status was found.</b><p class="muted">MediBridge hides stale emergency availability. Try a wider search. If this is urgent in India, call 112 or use your local emergency service rather than waiting for the app.</p></div>';
    msg('emergencyResultsMessage','No matching recently verified emergency hospital was found.','');
    return;
  }
  ...
  box.innerHTML=results.map(h=>{
    ...
    return `<div class="card hospital-result">
      <div class="row between">
        ...
        <span class="badge status-${escapeAdmin(status)}">${escapeAdmin(status)}</span>
      </div>
  ...
  if(context.mode==='radius'){
    msg('emergencyResultsMessage',
      `Showing recently verified matching hospitals within ${context.radius} km. Availability status is prioritised before distance and is hospital-reported.`,
      'success');
  }else{
    msg('emergencyResultsMessage',
      `District-wide results for ${context.district}. Only recently verified emergency statuses are shown; distance is intentionally not shown.`,
      'success');
  }
}
