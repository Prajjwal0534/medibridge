...
-- Run this AFTER v20-backend.sql.

-- 1) Upgrade session structure.
...
-- Translate old v20 state so existing test sessions do not break.
...
-- 4) Fetch one appointment session.
...
-- 5) Doctor starts/rings patient. Reusing the same appointment creates a NEW room after
-- a previous declined/missed/ended call, but keeps the same room while already active.
...
-- 7) A local user is marked joined ONLY after the embedded video provider confirms
-- videoConferenceJoined. When both are confirmed, state becomes connected.
...
begin
  ...
  if appt.patient_id = auth.uid() then
    ...
    set patient_joined_at = coalesce(patient_joined_at, now()),
        status = case
          when doctor_joined_at is not null then 'connected'
          else 'connecting'
        end,
        connected_at = case
    ...
    set doctor_joined_at = coalesce(doctor_joined_at, now()),
        status = case
          when patient_joined_at is not null then 'connected'
          else 'connecting'
        end,
        connected_at = case
...
-- 8) Doctor can cancel while ringing or end an active call.
...
-- 9) Frontend timeout can mark unanswered call missed, but only the assigned doctor.
