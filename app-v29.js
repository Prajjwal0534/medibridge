...
function aiModeSystemPrompt(mode){
  const prompts={
    patient_explain:`You are MediBridge AI for patients. Give general health information and explain medical terms in clear, calm, simple language. Help users prepare for professional care and understand how to use healthcare services. Do not claim a definite diagnosis. Do not present yourself as a doctor. If the message suggests potentially urgent or life-threatening symptoms, clearly advise immediate professional/emergency evaluation rather than relying on AI. Do not invent facts from a patient record that was not supplied in this chat.`,
    patient_questions:`You are MediBridge AI for patients. Help the user prepare useful, concise questions to ask a qualified healthcare professional. Explain why a question may matter when helpful. Do not diagnose. For potentially urgent symptoms, advise immediate professional/emergency care.`,
    doctor_reference:`You are MediBridge AI for verified clinicians. Provide concise clinical decision-support and reference assistance: differential brainstorming, terminology, report interpretation support, note drafting, follow-up questions, and literature-oriented summaries. Clearly distinguish established information from uncertainty. Do not act as an autonomous clinician and do not imply that output replaces professional judgment, local protocols, or direct patient assessment. Do not assume patient-specific facts unless explicitly provided in the current chat.`
  };
  return prompts[mode]||prompts.patient_explain;
}
...
function getAiConversation(){
  ...
  if(!mediBridgeAiChats[key]){
    mediBridgeAiChats[key]=[
      {role:'system',content:aiModeSystemPrompt(currentAiMode)}
    ];
  }
  ...
  if(!visible.length){
    box.innerHTML=`
      <div class="ai-chat-empty">
        <b>MediBridge AI</b>
        <p>Ask a ${currentProfile?.role==='doctor'?'clinical-reference':'health-information'} question. This temporary Puter chat stays in this browser session and is not written to Supabase.</p>
      </div>`;
    return;
  }
...
function setAiMode(mode,btn){
  ...
  const webSearchRow=document.getElementById('aiWebSearchRow');
  if(webSearchRow){
    webSearchRow.classList.toggle(
      'hidden',
      ['patient_summary','doctor_patient_review'].includes(mode)
    );
  }
...
function updateAiProviderStatus(){
  ...
  if(service.providerName()==='puter'){
    status.textContent='Temporary Puter provider enabled for general chat. No developer API key is stored in MediBridge.';
  }else{
...
async function askMediBridgeAI(){
  ...
  stopBtn.classList.toggle('hidden',Boolean(secureBackendMode));
  ...
  try{
    ...
    const useWebSearch=Boolean(document.getElementById('aiUseWebSearch')?.checked);

    msg(
      'aiMessage',
      useWebSearch
        ? 'Searching current information and generating a response...'
    ...
    const result=await window.MediBridgeAI.chatWithAI({
      ...
      stream:true,
      shouldStop:()=>mediBridgeAiGenerationStopped,
      onToken:(_delta,fullText)=>{
        if(streamingText){
          streamingText.innerHTML=renderAiMarkdown(fullText);
          const box=document.getElementById('aiChatThread');
    ...
    msg(
      'aiMessage',
      `Response generated${result?.model?` using ${result.model.replace('openai/','')}`:''}. Verify important medical information before use.`,
      'success'
    );
  }catch(err){
    renderAiChatThread();
    ...
    if(/load|undefined|not available/i.test(raw)){
      friendly='The temporary AI provider is unavailable right now. Please refresh and try again.';
      ...
      friendly='The selected AI model is temporarily unavailable. MediBridge will try configured fallback models automatically.';
    }
